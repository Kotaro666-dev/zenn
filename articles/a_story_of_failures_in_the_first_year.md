...
この記事では、僕がエンジニア 1 年目として主に Flutter 開発業務に携わってきた中での失敗談を 4 つ挙げます。

# コミュニケーションロスによる引き継ぎに失敗し、工数を無駄にした話

最初の失敗談は、エンジニアとして入社して間もない人ならやりがちなコミュニケーションロスから引き起こしたミスです。

入社して 2 つ目の調査課題を担当することになりました。

話の内容としては、以下のような感じでした。

> B さん（別の社内エンジニア）が以前に対応して修正したはずの不具合が先祖返り（デグレード）しているかもしれない。
> もしくは、勘違いで追加されていないだけかもしれない。
> とりあえず、調査してみてください。追加されてないものだったら、○○ さん（僕）が実装してくれないか。

ここで最初のミスは、この不具合修正を担当した B さんからではなく、第三者の PjM の A さんから依頼されて、課題を開始したことです。
つまり、 **当事者からの直接の引き継ぎを受けない状態でスタートした** ことです。
...
そこで、 多忙な B さんに VC で質問しに行きました。ここで、２つ目のミスがあります。

質問をしに行った際に、 **課題背景（先祖返りの可能性も含め）を説明する前に今回の問題を説明し始めてしまい、 B さんの頭をキャッチアップさせられなかった** ことです。

そのため、 B さんも困惑しながら僕の話を聞き、お互いで決まったことは「まだ実装されていない修正なら、僕がやってください」です。

そこで実装作業を進めていくと、すんなりと修正がうまくいきません。

まずプロジェクト全体の構造をしっかりと把握しておらず、「オブジェクト指向？聞いたことある」の実務未経験の僕にはそんな簡単に直せるはずもありません。
...
最終的には、別の強強エンジニアの人がマージ漏れが原因で発生していたことを発見し、この問題は解決しました。
...
# 自分がリリースした機能の影響で、端末によってアプリが白画面になる

ある程度業務に慣れてきた頃、端末のサウンドモードを検知してアプリ内の音量設定に反映させる課題がありました。

Flutter には便利な外部ライブラリが数多く存在し、こうしたネイティブ周りの機能も第三者が作成した外部ライブラリを使って解決することができることもあります。
...
原因は無駄な非同期処理実装を行っていて、古い端末によってはサウンドモード取得に失敗した場合に await して次の実行に移れていなかったようです。
...
# 口頭で受けた指示を実装したら、当初の要件と異なっていた場所で実装していた件
...
帰り支度を始めて、今日の振り返りをしていると、PjM から連絡。

「○○ さん（僕）、まだ帰宅しないでもらえますか？さっきのバナーの件、まだ追加されていないみたいです。」
...
お互いに話し合い、問題の画面をビデオ画面に見せあって、気づきました。

PjM と僕で思っていたバナーの表示箇所が異なっていました。

:::message
**ここでの学び**

1. 設計や実装に入る前に、まず要件をしっかりと確認する
2. 必ずしも特定の言葉が同じ意味で話しているのか分からないため、念入りに言葉の意味の摺り合わせをする
3. 要件に疑問点や不明点があれば、曖昧な状態でスタートさせない
4. 言葉で説明しづらいものは、ビジュアルツールに頼る

:::

# 終わりに

エンジニア 1 年目を振り返ってみると、会社の利益に直結するような取り返しのつかないミスをしなかったことは唯一の救いです。

ただし、自分が直接的に関与していないものの、頭の中が真っ白になるような本番での不具合に何度か遭遇しました。

その時に、先輩エンジニア達は今すべき行動をすぐに、そして冷静に考えて対処していたことが印象に残っています。

そうした冷静な対処ができるように日々経験を積んでいきたいです。
